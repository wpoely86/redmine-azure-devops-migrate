...
from azure.devops.v6_0.work_item_tracking import (
    JsonPatchOperation, CommentCreate, Link, CommentUpdate
)
...
def create_work_item(work_client, issue):
    ...
    orig_link = Link(rel="Hyperlink", url=f"{redmine_url}/issues/{issue.id}",
                     attributes={"comment": "link to original issue"})
...
"""
    description = markdown.markdown(fix_redmine_list_formatting(issue.description) + footer,
                                    extensions=['pymdownx.magiclink', 'pymdownx.extra'])
    ...
    if str(getattr(issue, 'assigned_to', 'Unassigned')) in author_map:
        ...
        new_work_item.append(
            JsonPatchOperation(op='Add', path='/fields/System.AssignedTo', value=default_user)
        )

    # bypass_rules is needed to create issues with a creation date in the past
    result = work_client.create_work_item(new_work_item, project, 'Issue',
                                          bypass_rules=True, suppress_notifications=True)
    ...
    with tempfile.TemporaryDirectory(suffix=f"-{issue.id}") as tmpdir:
        for attachment in issue.attachments:
            ...
            attach_link = Link(rel="AttachedFile", url=result_attach.as_dict().get('url'),
...
def main():
    ...
    issues = redmine.issue.filter(project_id='some_project', status_id='*', created_on='>=2017-01-01',
                                  sort='created_on', include='journals,attachments')
#    issues = [redmine.issue.get(x, include='journals,attachments') for x in [1, 2, 4]]
    ...
    for work_item_id, _ in issue_map.values():
        ...
        if update_item:
            update_item.append(JsonPatchOperation(op='Replace', path='/fields/System.Description',
